...
class ChordTransitioner:
    ...
    Transition = namedtuple('Transition', ['min_dist', 'cur_note', 'next_note'])
    ...
    def _min_diff(self, abs_note, rel_note):
    ...
    def _get_checking_priority(self, cur_abs_notes, next_rel_notes):
        ...
        for (cur_abs_note, next_rel_note) in product(cur_abs_notes, next_rel_notes):
            ...
            for note in min_diff_notes:
                new_transition = self.Transition(
                    min_dist=min_transition_diff,
                    cur_note=cur_abs_note,
                    next_note=self.KeyPosPair(
    ...
    def find_optimal_transition(self, cur_satb_chord, next_chord):
        ...
        return BFTransitionOptimizer(
            prioritized_checker,
            self.TransitionContext(cur_satb_chord, next_chord)
        ).solve()

    def _get_agg_min_cost_seqs(self, next_seqs):
    ...
    def _get_abs_min_cost_seqs(self, final_seqs):
    ...
    def _infer_init_note_pos(self, init_notes, init_model_chord):
        result = set()
        for init_note in init_notes:
            ...
            pair = self.KeyPosPair(scale_pos, Note(abs_note, int(parts.group(3))))
            if pair in result:
                raise ValueError('Duplicate note {} in initial notes.'.format(init_note))
            else:
                result.add(self.KeyPosPair(scale_pos, Note(abs_note, int(parts.group(3)))))
        return result

    def transition_chords(self, chord_seq, init_notes):
